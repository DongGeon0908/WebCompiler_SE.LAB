...
@WebServlet("/shellCompile_javascript")
public class shellCompile_javascript extends HttpServlet {
   ...
   protected void doHandle(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      try {
         ...
         if (check.equals("불법접근입니다.")) {
            ...
            FileWriter javascriptTest = new FileWriter("/usr/local/apache/share/javascriptTest.txt");
            javascriptTest.write(realCode);
            javascriptTest.close();
///
            ...
            if (length == 1) {
               shellCmd("sh /usr/local/apache/docker/basic/" + DN.get(0) + "/javascriptMid_" + DN.get(0) + ".sh");
               dockerCycle.write(DN.get(0));
               dockerCycle.close();
            } else if (length == 2) {
               if (docker.indexOf(DN.get(0)) != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/" + DN.get(0) + "/javascriptMid_" + DN.get(0)
                        + ".sh");
                  dockerCycle.write(DN.get(1));
                  dockerCycle.close();
               } else if (docker.indexOf(DN.get(1)) != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/" + DN.get(1) + "/javascriptMid_" + DN.get(1)
                        + ".sh");
                  dockerCycle.write(DN.get(0));
                  dockerCycle.close();
               }
               ...
               if (docker.indexOf("se01") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se01/javascriptMid_se01.sh");
                  dockerCycle.write("se02");
                  dockerCycle.close();
               } else if (docker.indexOf("se02") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se02/javascriptMid_se02.sh");
                  dockerCycle.write("se03");
                  dockerCycle.close();
               } else if (docker.indexOf("se03") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se03/javascriptMid_se03.sh");
            ...
            ////
            File javascriptResultNew = new File("/usr/local/apache/share/javascriptResult.txt");
            String logResult = null;
            if (javascriptResultNew.length() != 0) {
               String result = Reader("/usr/local/apache/share/javascriptResult.txt");
               out.print("<h2>Result</h2>");
               out.print("<h5>" + result + "</h5>");
               logResult = "success";
            } else {
               docker = "se01";
               String error = Reader("/usr/local/apache/share/javascriptError.txt");
