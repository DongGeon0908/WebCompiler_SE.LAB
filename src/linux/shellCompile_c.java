   ...
   protected void doHandle(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      try {
         ...
         if (check.equals("불법접근입니다.")) {
            ...
            FileWriter cTest = new FileWriter("/usr/local/apache/share/cTest.txt");
            cTest.write(realCode);
            cTest.close();
            ...
            if (length == 1) {
               shellCmd("sh /usr/local/apache/docker/basic/" + DN.get(0) + "/cMid_" + DN.get(0) + ".sh");
               ...
               if (docker.indexOf(DN.get(0)) != -1) {
                  ...
                  shellCmd("sh /usr/local/apache/docker/basic/" + DN.get(1) + "/cMid_" + DN.get(1) + ".sh");
                  dockerCycle.write(DN.get(0));
                  dockerCycle.close();
               }
            } else if (length == 3) {
               if (docker.indexOf("se01") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se01/cMid_se01.sh");
                  dockerCycle.write("se02");
                  dockerCycle.close();
               } else if (docker.indexOf("se02") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se02/cMid_se02.sh");
                  dockerCycle.write("se03");
                  dockerCycle.close();
               } else if (docker.indexOf("se03") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se03/cMid_se03.sh");
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            File cResultNew = new File("/usr/local/apache/share/cResult.txt");

            String logResult = null;
            if (cResultNew.length() != 0) {
               String result = Reader("/usr/local/apache/share/cResult.txt");
               out.print("<h2>Result</h2>");
               out.print("<h5>" + result + "</h5>");
               logResult = "success";
            } else {
               String error = Reader("/usr/local/apache/share/cError.txt");
