...
@WebServlet("/shellCompile_python")
   ...
   protected void doHandle(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      try {
         ...
         if (check.equals("불법접근입니다.")) {
            ...
            FileWriter pythonTest = new FileWriter("/usr/local/apache/share/pythonTest.txt");
            pythonTest.write(realCode);
            pythonTest.close();
///
            ...
            if (length == 1) {
               shellCmd("sh /usr/local/apache/docker/basic/" + DN.get(0) + "/pythonMid_" + DN.get(0) + ".sh");
               ...
               if (docker.indexOf(DN.get(0)) != -1) {
                  ...
                  shellCmd("sh /usr/local/apache/docker/basic/" + DN.get(1) + "/pythonMid_" + DN.get(1) + ".sh");
                  dockerCycle.write(DN.get(0));
                  dockerCycle.close();
               }
            } else if (length == 3) {
               if (docker.indexOf("se01") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se01/pythonMid_se01.sh");
                  dockerCycle.write("se02");
                  dockerCycle.close();
               } else if (docker.indexOf("se02") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se02/pythonMid_se02.sh");
                  dockerCycle.write("se03");
                  dockerCycle.close();
               } else if (docker.indexOf("se03") != -1) {
                  shellCmd("sh /usr/local/apache/docker/basic/se03/pythonMid_se03.sh");
            ...
            ///
            File pythonResultNew = new File("/usr/local/apache/share/pythonResult.txt");

            String logResult = null;
            if (pythonResultNew.length() != 0) {
               String result = Reader("/usr/local/apache/share/pythonResult.txt");
               out.print("<h2>Result</h2>");
               out.print("<h5>" + result + "</h5>");
               logResult = "success";
            } else {
               docker = "se01";
               String error = Reader("/usr/local/apache/share/pythonError.txt");
